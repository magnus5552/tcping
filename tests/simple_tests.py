...
class SimpleTests(unittest.TestCase):
    ...
    def test_simple(self):
        self.ping.ping()
        self.assertTrue(all(r.is_success for r in self.fake_socket.statistic.results))

    def test_handle_timeout(self):
        self.fake_socket.fake_timeout = 0.2
        self.ping.ping()
        self.assertTrue(all(r.is_timeout for r in self.fake_socket.statistic.results))

    def test_handle_OSError(self):
        self.fake_socket.is_OSError = True
        self.ping.ping()
        self.assertTrue(all(not r.is_success and not r.is_timeout
                            for r in self.fake_socket.statistic.results))
